...
Glow overlay images SHALL have `opacity: 0` by default and SHALL transition to `opacity: 1` with a CSS animation when their parent `.glow` element receives the `.visible` class.

#### Scenario: Glow becomes visible
- **WHEN** `.glow.visible` class is applied
- **THEN** child `<img>` SHALL have `opacity: 1`
- **AND** SHALL animate with `zoomer` keyframe animation looping every 2 seconds
