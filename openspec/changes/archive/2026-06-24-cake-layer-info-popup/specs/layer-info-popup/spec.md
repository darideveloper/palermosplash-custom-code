...
## Purpose
Defines the click-based popup that surfaces the detailed information of an active cake layer: icon, full name, description, and an "Explore" link. The popup is a single, fixed-positioned element that swaps content and position based on the active layer.
